...
- **`/atv-doctor` and `/atv-update` skills** — diagnose ATV install health and update marketplace plugins. New `🩺 Maintenance & Health` TUI category. `/atv-doctor` checks both install paths (project scaffold + marketplace), reports version drift, file integrity via checksum manifest, MCP prereqs, and gated optional-dep status. `/atv-update` auto-updates marketplace plugins with confirmation; project scaffold path is advisory (installer is additive-only today).
- **Copilot CLI plugin marketplace** — ATV now ships as a marketplace at `.github/plugin/marketplace.json`. Install personally with `copilot plugin marketplace add All-The-Vibes/ATV-StarterKit` then `copilot plugin install atv-everything@atv-starter-kit` (or pick category packs / single skills). See [docs/marketplace.md](https://github.com/All-The-Vibes/ATV-StarterKit/blob/main/docs/marketplace.md).
